<x-profile-layout>
    ...
    <div class="bg-white p-6 rounded-xl shadow-soft">
        ...
        <div x-data="sortableComponent('{{ route('referee.reorder') }}')">
                ...
                @forelse ($referees as $referee)
                    <div class="sortable-item p-4 border rounded-lg flex justify-between items-center bg-white cursor-grab active:cursor-grabbing"
                        ...
                        <div class="flex items-center space-x-2 shrink-0 ml-4">
                            <a href="{{ route('referee.edit', $referee) }}" class="flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 hover:bg-primary-100 text-primary-600">
                                <i class="fas fa-pencil-alt"></i>
                            </a>
                            <form action="{{ route('referee.destroy', $referee) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                                @csrf @method('DELETE')
