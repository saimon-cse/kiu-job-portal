<x-profile-layout>
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h2 class="text-xl font-bold text-gray-800">My Awards & Recognitions</h2>
            <p class="text-gray-500">Drag and drop any item to change its display order.</p>
        </div>
        <a href="{{ route('award.create') }}" class="bg-primary-600 hover:bg-primary-700 text-white py-2 px-4 rounded-lg font-medium text-sm">
    ...
    <div class="bg-white p-6 rounded-xl shadow-soft">
        ...
        <div x-data="sortableComponent('{{ route('award.reorder') }}')">
            ...
            <div id="sortable-list" class="space-y-4">
                ...
                @forelse ($awards as $award)
                    {{-- The 'sortable-item' class enables the smooth CSS transition --}}
                    <div class="sortable-item p-4 border rounded-lg flex justify-between items-center bg-white cursor-grab active:cursor-grabbing"
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                <h3 class="font-bold text-lg text-gray-800">{{ $award->award_name }}</h3>
                                <p class="text-gray-600">Awarded by: {{ $award->awarding_body }}</p>
                                <p class="text-sm text-gray-500 mt-1">
                                    Year: {{ $award->year_received }}
                                    @if ($award->country)
                                        <span class="mx-1">|</span> Country: {{ $award->country }}
                                    @endif
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center space-x-2 shrink-0 ml-4">
                            <div class="relative group">
                                <a href="{{ route('award.edit', $award) }}" class="flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 hover:bg-primary-100 text-primary-600">
                            ...
                            <div class="relative group">
                                <form action="{{ route('award.destroy', $award) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this award?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <div class="text-center py-8">
                        <i class="fas fa-award text-4xl text-gray-300"></i>
                        <p class="mt-4 text-gray-500">You haven't added any awards or recognitions yet.</p>
                        {{-- <a href="{{ route('award.create') }}" class="mt-4 inline-block text-primary-600 hover:underline">Click here to add one.</a> --}}
                    </div>
                @endforelse
            </div>
